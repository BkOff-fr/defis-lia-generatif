<script lang="ts">
  ...
  async function onSubmit(): Promise<void> {
    if (!backendAvailable || !formValid || saving) return;
    saving = true;
    formError = null;
    try {
  ...
  function errorHelp(code: string): string {
    switch (code) {
      ...
      case 'tauri_unavailable':
        return "L'éco-budget se suit depuis votre ledger local, dans l'application de bureau Sobr.ia.";
      default:
        return '';
    }
  }
...
<div class="canvas-inner">
  ...
  <section class="hero">
    <div class="hero-eyebrow">
      ...
      Éco-budget personnel
    </div>
    <h1 class="hero-h1">
      Pose un <em>budget</em>, suis l'impact.
    </h1>
    <p class="hero-sub">
      Définissez vos propres seuils mensuels, hebdomadaires ou journaliers — sur le CO₂eq, l'énergie
      ou l'eau. Sobr.ia mesure votre consommation réelle (P50 Monte-Carlo) et vous alerte avant que
      vous n'ayez dépassé.
    </p>
  </section>
  ...
  {#if !backendAvailable}
    <div class="banner" data-tone="warn" role="alert">
      ...
      <div class="banner-body">
        ...
        <span>
          L'éco-budget nécessite l'application de bureau Sobr.ia. Les objectifs sont stockés dans la
          base SQLite locale — pas de serveur, pas d'envoi externe.
        </span>
      </div>
    </div>
  {/if}
      ...
      >
        <label class="field">
          ...
          <select
            ...
            disabled={!backendAvailable || saving}
            class="select-input"
        ...
        <button
          type="submit"
          class="btn-primary"
          disabled={!backendAvailable || saving || !formValid || !desktopAvailable}
            ...
            <li class="budget-item" data-status={st.status}>
              <div class="bi-head">
                ...
                <button
                  ...
                  disabled={!backendAvailable || deletingKey === keyOf(st.goal)}
