<script lang="ts">
  ...
  async function submitCreate(): Promise<void> {
    if (!backendAvailable || !createValid) return;
    submitting = true;
    formError = null;
    try {
  ...
  async function submitEdit(): Promise<void> {
    if (!backendAvailable || !editValid || panel.kind !== 'edit') return;
    submitting = true;
    formError = null;
  ...
  async function confirmDelete(): Promise<void> {
    if (!backendAvailable || confirmDeleteId === null) return;
  ...
  async function generateCsrd(): Promise<void> {
    if (!backendAvailable || !datasheet) return;
    csrdState = 'pending';
    csrdMsg = null;
    try {
  ...
  function errorHelp(code: string): string {
    switch (code) {
      ...
      case 'tauri_unavailable':
        return "L'empreinte projet se calcule depuis votre ledger local, dans l'application de bureau Sobr.ia.";
      default:
        return '';
    }
  }
...
<div class="canvas-inner">
  ...
  <section class="hero">
    ...
    <p class="hero-sub">
      Regroupez vos estimations en projets nommés et générez leur datasheet selon le standard
  ...
  {#if !backendAvailable}
    <div class="banner" data-tone="warn" role="alert">
      ...
      <div class="banner-body">
        ...
        <span>
          Les projets et datasheets nécessitent l'application de bureau Sobr.ia. Ils restent 100 %
      ...
      {:else if projects.length === 0}
        <div class="empty-state">
          ...
          <p class="empty-body">
            ...
            {:else}
              Installez l'application de bureau pour charger vos projets et générer leurs
              datasheets.
            {/if}
          </p>
        </div>
      ...
      {:else if panel.kind === 'datasheet'}
        ...
        {:else if datasheet}
          ...
          <div
            ...
            title="Format standard pour documenter datasets et modèles ML, publié par Gebru, Morgenstern, Vecchione et al. en 2018. Adopté par NeurIPS, ICML, FAccT. Sobr.ia génère ce format automatiquement depuis votre ledger d'audit, pour faciliter la publication scientifique reproductible."
          >
          ...
          <div class="ds-actions">
            ...
            <button
              ...
              disabled={!backendAvailable || saveState === 'saving'}
            >
              {#if saveState === 'saving'}
                <Loader2 size={14} strokeWidth={2} class="spin" /> Enregistrement…
            ...
            <button
              ...
              disabled={!backendAvailable || csrdState === 'pending'}
            >
