<script lang="ts">
  ...
  function errorHelp(code: string): string {
    ...
    if (code === 'tauri_unavailable') {
      return "Le tableau de bord complet se nourrit de votre ledger local, dans l'application de bureau Sobr.ia.";
    }
    return '';
  }
...
<div class="canvas-inner">
  ...
  <section class="hero">
    ...
    <h1 class="hero-h1">
      Votre <em>empreinte IA</em>, période après période.
    </h1>
    <p class="hero-sub">
      Visualisez vos requêtes, CO₂eq, énergie et eau cumulés sur 5 fenêtres temporelles — et
      comparez vs la période précédente. Les chiffres sont calculés en P50 depuis le ledger d'audit
      local.
    </p>
  </section>
  ...
  {#if !backendAvailable}
    <div class="banner" data-tone="warn" role="alert">
      ...
      <div class="banner-body">
        ...
        <span>
          Le tableau de bord complet nécessite l'application de bureau Sobr.ia. Le ledger d'audit
          reste 100 % local — aucun envoi externe.
        </span>
      </div>
    </div>
  {/if}
  ...
  <div id="m15-panel" role="tabpanel" aria-labelledby="m15-tab-{period}" tabindex="0" class="panel">
    ...
    <section class="card">
      ...
      {#if !summary}
        <p class="empty">
          {backendAvailable
            ? 'Aucune donnée encore — sélectionnez une période.'
            : "Installez l'application de bureau pour charger votre ledger d'audit."}
        </p>
      {:else if summary.daily_series.length === 0}
        <div class="empty-welcome">
          <p class="empty">
            Aucune mesure sur cette période — votre tableau de bord se remplit tout seul dès que
            vous mesurez.
          </p>
          <div class="empty-ctas">
            <a class="empty-cta" href="/">Estimer un premier prompt</a>
            <a class="empty-cta" href="/parametres">Associer l'extension (mesure automatique)</a>
          </div>
        </div>
      {:else}
        ...
        <ol class="top-list">
          {#each summary.top_models as m, i (m.model_id)}
            ...
            <li class="top-row">
              ...
              <a
                class="model-link"
                href="/modeles?id={encodeURIComponent(m.model_id)}"
    ...
    <footer class="pfoot">
      ...
      {:else}
        <span class="pf-label">
          ...
          Les chiffres se chargeront depuis votre ledger d'audit dans l'application de bureau.
        </span>
      {/if}
    </footer>
  </div>
